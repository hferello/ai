#!/bin/bash
# Install feature skill to ~/.cursor/skills/ (global, all projects)
...
if [[ -f "$SCRIPT_DIR/SKILL.md" ]]; then
  ...
  echo "Error: Could not find SKILL.md. Run this from the feature skill directory."
  exit 1
fi
...
echo "  2. In Agent chat, type /feature or @feature"
echo ""
echo "To initialize planning files in a project, run from project root:"
echo "  ~/.cursor/skills/feature/scripts/init-session.sh <task-name>"
echo ""
